...
def assign_firestation_to_key_locs():
    ...
    for key_site in key_sites:
        ...
        for station in fire_stations:
            ...
            dist = get_distance(
                station[0], int(key_site[0]), station[1], int(key_site[1])
            )

            if min > dist:
                min = dist
                station_id = (
                    str(station[2]) + "_" + str(station[0]) + "_" + str(station[1])
                )
                min_coord = {
                    "station": [station[0], station[1], station[2]],
                    "key_site": [key_site[0], key_site[1], key_site[2]],
