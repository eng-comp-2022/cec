...
def generateMap(saveMap, filePath):
    ...
    fig.update_layout(
        ...
        title = go.layout.Title(
            text = "Optimized Firetower Placement"),
        ...
        geo = go.layout.Geo(
            ...
            showlakes=True, lakecolor="Blue", #Show lakes to get a better view of NB
            showrivers=True, rivercolor="Blue", #Show rivers as SJ river is a big fixture of NB
            showocean=True, oceancolor="LightBlue", #Shows ocean as NB is on the coast
            center=dict(lon=-66.170557, lat=46.652598), #Center the view on the center of NB
            lonaxis_range= [ -75, -57 ], #Set the default range of the longitude to be zoomed to NB, this is ~1386km
            lataxis_range= [45, 47], #Set the default range of the latitude to be zoomed to NB, this is ~222km
    ...
    if saveMap == True:
        fig.write_image(filePath + "/fireTowerMap.png") #Uses kaleido (pip install -U kaleido)
...
def generateText(filePath):
    ...
    with open( filePath + "/fireTowerPlacement.txt", 'w') as f:
        ...
        for i in range(len(data)):
            f.write("City: " + data[i]["City"] + ", Type " + str(data[i]["Type"]) + "\n\t" + "Latitude: " + str(data[i]["Lat"]) + "\n\t" + "Longitude: " + str(data[i]["Lon"]) + "\n")